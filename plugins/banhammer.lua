...
if is_banned(data.id_, arg.chat_id) then
   if not lang then
		tdcli.sendMessage(arg.chat_id, arg.msg_id, 0, "_User_ "..user_name.." *[ "..data.id_.." ]* _is banned🔏_", 0, "md")
   else
		tdcli.sendMessage(arg.chat_id, arg.msg_id, 0, "_کاربر_ "..user_name.." *[ "..data.id_.." ]* _از گروه محروم است🔏_", 0, "md")
end
kick_user(data.id_, arg.chat_id)
end
if is_gbanned(data.id_) then
     if not lang then
		tdcli.sendMessage(arg.chat_id, arg.msg_id, 0, "_User_ "..user_name.." *[ "..data.id_.." ]* _is globally banned🔏_", 0, "md")
    else
...
user_name = check_markdown(data.first_name_)
  ...
  if not lang then
  return tdcli.sendMessage(arg.chat_id, "", 0, "_You can't_ *globally ban* _other admins🔏_", 0, "md")
  else
...
if not matches[2] and tonumber(msg.reply_to_message_id_) ~= 0 then
  ...
  if matches[2] and string.match(matches[2], '^%d+$') then
   ...
   if not lang then
     tdcli.sendMessage(msg.chat_id_, "", 0, "_You can't kick mods,owners or bot admins🔏_", 0, "md")
   elseif lang then
...
if not matches[2] and tonumber(msg.reply_to_message_id_) ~= 0 then
  ...
  if matches[2] and string.match(matches[2], '^%d+$') then
   if is_mod1(msg.chat_id_, matches[2]) then
   if not lang then
   return tdcli.sendMessage(msg.chat_id_, "", 0, "_You can't delete messages mods,owners or bot admins🔏_", 0, "md")
     elseif lang then
...
if not matches[2] and tonumber(msg.reply_to_message_id_) ~= 0 then
  ...
  if matches[2] and string.match(matches[2], '^%d+$') then
   ...
   if not lang then
    return tdcli.sendMessage(msg.chat_id_, "", 0, "_You can't globally ban other admins🔏_", 0, "md")
else
...
if not matches[2] and tonumber(msg.reply_to_message_id_) ~= 0 then
  ...
  if matches[2] and string.match(matches[2], '^%d+$') then
   if is_mod1(msg.chat_id_, matches[2]) then
     if not lang then
    return tdcli.sendMessage(msg.chat_id_, "", 0, "_You can't ban mods,owners or bot admins🔏_", 0, "md")
    else
   ...
   if not lang then
  return tdcli.sendMessage(msg.chat_id_, "", 0, "_User "..matches[2].." is already banned🔏_", 0, "md")
  else
...
kick_user(matches[2], msg.chat_id_)
   if not lang then
 return tdcli.sendMessage(msg.chat_id_, msg.id_, 0, "_User "..matches[2].." has been banned🔏_", 0, "md")
  else
...
if not matches[2] and tonumber(msg.reply_to_message_id_) ~= 0 then
  ...
  if matches[2] and string.match(matches[2], '^%d+$') then
   ...
   if not lang then
   return tdcli.sendMessage(msg.chat_id_, "", 0, "_User "..matches[2].." is not banned🔏_", 0, "md")
  else
...
data[tostring(chat)]['banned'][tostring(matches[2])] = nil
    save_data(_config.moderation.data, data)
   if not lang then
return tdcli.sendMessage(msg.chat_id_, msg.id_, 0, "_User "..matches[2].." has been unbanned🔏_", 0, "md")
   else
...
if not matches[2] and tonumber(msg.reply_to_message_id_) ~= 0 then
  ...
  if matches[2] and string.match(matches[2], '^%d+$') then
   if is_mod1(msg.chat_id_, matches[2]) then
   if not lang then
   return tdcli.sendMessage(msg.chat_id_, "", 0, "_You can't silent mods,leaders or bot admins🔏_", 0, "md")
 else
   ...
   if not lang then
   return tdcli.sendMessage(msg.chat_id_, "", 0, "_User "..matches[2].." is already silent🔏_", 0, "md")
   else
...
data[tostring(chat)]['is_silent_users'][tostring(matches[2])] = ""
    save_data(_config.moderation.data, data)
    if not lang then
 return tdcli.sendMessage(msg.chat_id_, msg.id_, 0, "_User "..matches[2].." added to silent users list🔏_", 0, "md")
  else
...
if not matches[2] and tonumber(msg.reply_to_message_id_) ~= 0 then
  ...
  if matches[2] and string.match(matches[2], '^%d+$') then
   if not is_silent_user(matches[2], chat) then
     if not lang then
     return tdcli.sendMessage(msg.chat_id_, "", 0, "_User "..matches[2].." is not silent🔏_", 0, "md")
   else
...
data[tostring(chat)]['is_silent_users'][tostring(matches[2])] = nil
    save_data(_config.moderation.data, data)
   if not lang then
 return tdcli.sendMessage(msg.chat_id_, msg.id_, 0, "_User "..matches[2].." removed from silent users list🔏_", 0, "md")
  else
 return tdcli.sendMessage(msg.chat_id_, msg.id_, 0, "*کاربر "..matches[2].." توانایی چت کردن رو به دست آورد🔏*", 0, "md")
  ...
  if matches[2] and not string.match(matches[2], '^%d+$') then
   tdcli_function ({
     ...
     if not lang then
					return "_No_ *banned* _users in this group🔏_"
   else
     ...
     if not lang then
				return "_All_ *banned* _users has been unbanned🔏_"
    else
        ...
        if not lang then
					return "_No_ *silent* _users in this group🔏_"
   else
       ...
       if not lang then
				return "*Silent list* _has been cleaned🔏_"
   else
    ...
    if not lang then
					return "_No_ *globally banned* _users available🔏_"
   else
      ...
      if not lang then
				return "_All_ *globally banned* _users has been unbanned🔏_"
   else
